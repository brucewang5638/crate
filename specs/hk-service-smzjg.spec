...
%define _logfile        /var/log/hk-service-smzjg-install.log

# --- 接收来自命令行的参数 ---
# %{!?pkg_name: %global pkg_name hk-smzjg} -> 由 Builder 传入 comp_name
# %{!?version: %global version 1.0.0} -> 由 Builder 传入 version

Name:           hk-service-smzjg
Version:        %{version}
Release:        1%{?dist}
Summary:        HK 自监管业务应用主程序 - smzjg
...
# 3. 安装 Procmate 配置文件
install -d -m 755 %{buildroot}%{_procmate_dir}
install -m 644 config/hk-service-smzjg.yaml %{buildroot}%{_procmate_dir}/hk-service-smzjg.yaml
...
%preun
if [ $1 -eq 0 ]; then
    rm -f %{_procmate_dir}/hk-service-smzjg.yaml
    systemctl try-restart procmate.service >/dev/null 2>&1
fi
...
%{_installpath}/res
%{_installpath}/script
%config(noreplace) %{_procmate_dir}/hk-smzjg.yaml
